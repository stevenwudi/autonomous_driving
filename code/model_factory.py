...
class Model_Factory():
    ...
    def basic_model_properties(self, cf, variable_input_size):
        ...
        if cf.dataset.class_mode == 'categorical':
            ...
            # TODO detection : check model, different detection nets may have different losses and metrics (DONE)

            print ('   Using %s losses and metrics ...' % (cf.model_name))
            ...
            if 'yolo' in cf.model_name:
                ...
                in_shape = (cf.target_size_train[0], cf.target_size_train[1], cf.dataset.n_channels) # important
                loss = SSDLoss(cf.dataset.n_classes + 1, neg_pos_ratio=2.0)
                metrics = [SSDMetrics()]
                # metrics = None
                # priors = pickle.load(open('priors/prior_boxes_ssd300.pkl', 'rb'))
                # loss = SSDLoss(cf.dataset.n_classes, cf.dataset.priors)

            else:
                raise ValueError('Unknown detection problem type')
    ...
    def make_one_net_model(self, cf, in_shape, loss, metrics, optimizer):
        ...
        # Compile model
        model.compile(loss=loss, metrics=metrics, optimizer=optimizer) #neg_pos_ratio=2.0)
