...
from code_base.tools.CityScape_dataloader import CityscapesDataset, num_classes, full_to_colour, train_to_full
...
train_dataset = CityscapesDataset(split='train', crop=args.crop_size, flip=True, dataroot=dataroot_dir)
val_dataset = CityscapesDataset(split='val', dataroot=dataroot_dir)
...
def test(e):
  ...
  for i, (input, _, target) in enumerate(val_loader):
    ...
    if i % 25 == 0:
      ...
      save_image(pred_colour[0].float().div(255), os.path.join(exp_dir, str(e) + '_' + str(i) + '.png'))
  ...
  torch.save(net.state_dict(), os.path.join(exp_dir, str(e) + '_net.pth'))
  torch.save(scores, os.path.join(exp_dir, 'scores.pth'))
  ...
  plt.xlabel('Epoch')
  plt.ylabel('Mean IoU')
  plt.savefig(os.path.join(exp_dir, 'ious.png'))
  plt.close()
